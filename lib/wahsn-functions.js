module.exports = (function() {
  ...
  function addSharePractice(practice, shareList, output, context) {
    output.debug("Sharing %s.", practice.accountName);
    console.log("Sharing "+practice.accountName);
    return Promise.each(shareList, (entry) => {
      ...
      console.log("adding "+entry.permission+" share for "+practice.accountName+" to "+entry.datasetId);
  ...
  // Add sharing to the share dataset list
  function addShare(input, output, context) {
    let practiceList = [];
    let shareList = [];
    ...
    return context.tdxApi.getDatasetDataAsync(input.datasetAccounts)
      ...
      .then((response) => {
        ...
        output.debug("got %d share datasets", shareList.length);
    	  return Promise.each(practiceList, (practice) => {
          return addSharePractice(practice, shareList, output, context);
        });
  	  });
  }
